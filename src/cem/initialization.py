...
class CEMInitConfig:
    ...
    def _validate_len_bin_seq(self) -> None:
        ...
        if self.len_bin_seq < 16 or self.len_bin_seq > 4096:
            raise ValueError(
                f"len_bin_seq must be in [16, 4096]: {self.len_bin_seq}"
            )
    ...
    def _validate_method_dist(self) -> None:
        ...
        if self.method_dist not in cm.DIST_METHODS:
            raise ValueError(
                f"method_dist must be one of {cm.DIST_METHODS}:'{self.method_dist}'"
            )
...
class CEMInitResult:
    ...
    logger : logger.ExperimentLogger
...
class CEMInitializer:
    ...
    def _create_log_directory(
        ...
        method_dir = self.base_dir / cm.LOG_DIR_NAME / self.experiment_name / method_dist
        method_dir.mkdir(parents=True, exist_ok=True)

        method_len_dir = method_dir / str(len_bin_seq)
        method_len_dir.mkdir(parents=True, exist_ok=True)

        # Create timestamped subdirectory if method_dist is "naive"
